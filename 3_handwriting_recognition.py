...
#train_data = gluon.data.DataLoader(train_ds, batch_size, shuffle=True, last_batch="rollover", num_workers=multiprocessing.cpu_count()-2)
#test_data = gluon.data.DataLoader(test_ds, batch_size, shuffle=True, last_batch="keep", num_workers=multiprocessing.cpu_count()-2)
...
figs_to_plot = len(images)
fig, axs = plt.subplots(figs_to_plot, figsize=(8, 1.3*figs_to_plot))

for i,image1,filename in zip(range(figs_to_plot),images,filenames):
    n = int(random.random()*len(test_ds))
    image, actual_label = test_ds[n]
    image = image1
    image = cv2.resize(image,(800,60))
    gray = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    ret, image = cv2.threshold(gray, 255,255,cv2.THRESH_TRUNC)
    ...
    f= open(os.path.join(cwd,outpath,filename+'.txt'),"w+")
    f.write(decoded_prediction_text)
